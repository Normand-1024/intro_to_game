...
int main(int argc, char *argv[])
{
	...
	explodeSound = Mix_LoadWAV("explode.wav");
	Mix_Music* backgroundMusic;
	backgroundMusic = Mix_LoadMUS("reallyWeirdStarWarsAndTwightlightZoneTheme.mp3");

	Mix_VolumeChunk(shootingSound, 60);
	Mix_VolumeChunk(explodeSound, 60);
	...
	while (!done) {
		...
		for (GameObject& crate : Crates)
			if (crate.active) active_crates++;
		if (active_crates < 10) active_crates = 10;
		int chance = (int)(100.0 * ((double)(active_crates) / 56.0));

		if (GameMode == GAME) {
			for (GameObject& crate : Crates) {
				if (crate.active && rand() % chance == 0) {
					if (rand() % chance == 0) {
						for (GameObject& bullet : Bullets) {
							if (!bullet.active) {
								bullet.position = Vec3(crate.position);
								bullet.position.y -= crate.scale.y + 0.05f;
								bullet.tag = ENEMY_BULLET;
								bullet.active = true;
								bullet.sprite.textureID = lT;
								if (bullet.velocity.y > 0) bullet.velocity.y *= -1;
								break;
							}
						}
